#!/usr/bin/env npx tsx
/**
 * 쇼핑탭 순위 체크 - Supabase 19개 상품 전체
 *
 * 흐름:
 * 1. PC 네이버 접속
 * 2. 메인 키워드 검색
 * 3. 쇼핑 더보기 클릭 → 쇼핑탭 진입
 * 4. 현재 페이지에서 MID 매칭 (광고 포함)
 * 5. 없으면 → 다음 페이지 (최대 15페이지)
 * 6. 있으면 → (페이지-1)*40 + 순위 = 최종 순위
 */
...
async function findRankInShopping(
  ...
  console.log(`    쇼핑탭 URL: ${currentUrl.substring(0, 50)}...`);
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const newRank = await findRankInShopping(page, product.keyword, product.mid);
